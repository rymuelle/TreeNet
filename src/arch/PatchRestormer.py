...
class TiledMDTA(nn.Module):
    def __init__(self, channels, num_heads, window_size, patch_attention=False):
        super().__init__()
        self.num_heads = num_heads
        self.window_size = window_size
        self.temperature = nn.Parameter(torch.ones(1, num_heads, 1, 1, 1))
    ...
    def forward(self, x):
        ...
        q = rearrange(q, 'b (head c) (h ph) (w pw) -> b head (h w) c (ph pw)', 
                      head=self.num_heads, ph=self.window_size, pw=self.window_size)
        k = rearrange(k, 'b (head c) (h ph) (w pw) -> b head (h w) c (ph pw)', 
                      head=self.num_heads, ph=self.window_size, pw=self.window_size)
        v = rearrange(v, 'b (head c) (h ph) (w pw) -> b head (h w) c (ph pw)', 
                      head=self.num_heads, ph=self.window_size, pw=self.window_size)
