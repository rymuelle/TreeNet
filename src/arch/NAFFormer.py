...
class NAFFormer(nn.Module):
    def __init__(self, 
        ...
        self.encoder3 = nn.Sequential(*[
            NAFBlock(c=int(dim * 2 ** 2)) for i in range(spatial_num_blocks[1])])

        self.d3_4 = Downsample(int(dim * 2 ** 2))  ## From Level 3 to Level 4
        self.s_latent = nn.Sequential(*[
            NAFBlock(c=int(dim * 2 ** 3)) for i in range(spatial_num_blocks[1])])
        ...
        self.decoder1 = nn.Sequential(*[
            NAFBlock(c=int(dim)) for i in range(spatial_num_blocks[1])])
        #####################################  end  ##################################### 
