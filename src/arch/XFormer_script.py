...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        red_c = c // self.num_heads
        q = q.view(b, self.num_heads, red_c, h, w ).view(b, self.num_heads, red_c, h*w)
        k = k.view(b, self.num_heads, red_c, h, w ).view(b, self.num_heads, red_c, h*w)
        v = v.view(b, self.num_heads, red_c, h, w ).view(b, self.num_heads, red_c, h*w)
        ...
        out = (attn @ v)
        out = out.view(b, self.num_heads, red_c, h*w).view(b, c, h, w)
